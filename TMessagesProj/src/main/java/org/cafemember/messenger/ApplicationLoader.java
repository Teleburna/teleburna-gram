...
import org.cafemember.ui.Components.ForegroundDetector;
...
public class ApplicationLoader extends Application {
    ...
    public static volatile int viewCoins;
    public static volatile int joinCoins;
    public static void animationTextView(final TextView gemCountTextView, final int start, final int end){
        ValueAnimator mValueAnimator = null;
        gemCountTextView.setTypeface(FontManager.instance().getTypeface());
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB && end > start) {
            mValueAnimator = ValueAnimator.ofFloat(start, end);

            final int duration = Math.min(5000, Math.abs(start - end) * 30);
            ...
            mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator valueAnimator) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                        current = Float.valueOf(valueAnimator.getAnimatedValue().toString());
                    }
                    gemCountTextView.setText((int) current + "");
                }
            });


            final int colorFrom = applicationContext.getResources().getColor(android.R.color.white);
            int colorTo1 = applicationContext.getResources().getColor(R.color.colorOrange);
            int colorTo2 = applicationContext.getResources().getColor(android.R.color.black);
            ValueAnimator colorAnimation = null;
            if (end > start) {
                colorAnimation = ValueAnimator.ofObject(new ArgbEvaluator(), colorFrom, colorTo1);
            } else {
                colorAnimation = ValueAnimator.ofObject(new ArgbEvaluator(), colorFrom, colorTo2);
            }
            colorAnimation.setDuration(400); // milliseconds
            colorAnimation.setRepeatCount(duration / 400);
            ...
            colorAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animator) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
                        gemCountTextView.setTextColor((int) animator.getAnimatedValue());
                    }
                    else {
                        gemCountTextView.setTextColor(colorFrom);
                    }
                }
            ...
            colorAnimation.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animator) {
                    gemCountTextView.setTextColor(applicationContext.getResources().getColor(android.R.color.white));
                }
            ...
            gemCountTextView.setText(end+"");
        }
    }
    public static void setViewCoins(int coins){
        if(DialogsActivity.viewCoins != null){
            animationTextView(((TextView) DialogsActivity.viewCoins.findViewById(R.id.total)),viewCoins,coins);
        }
        if(ChatActivity.viewCoins != null){
            animationTextView(((TextView) ChatActivity.viewCoins.findViewById(R.id.total)),viewCoins,coins);
        }
        viewCoins = coins;

    }
    public static void setJoinCoins(int coins){
        setJoinCoins(coins, false);

    }
    public static void setJoinCoins(int coins, boolean immidiate){
        if(DialogsActivity.joinCoins != null){
            if(!immidiate) {
                animationTextView(((TextView) DialogsActivity.joinCoins.findViewById(R.id.total)), joinCoins, coins);
            }
            else {
                ((TextView) DialogsActivity.joinCoins.findViewById(R.id.total)).setText(coins+"");
    ...
    public static File getFilesDirFixed() {
        ...
        return new File("/data/data/org.cafemember.messenger/files");
    }
    ...
    public void onCreate() {
        ...
        FontManager.init(applicationContext,R.string.font_iransans);
