...
import org.cafemember.ui.Components.TypefaceSpan;
...
public class ChatBaseCell extends BaseCell implements MediaController.FileDownloadProgressListener {
    ...
    public interface ChatBaseCellDelegate {
        ...
        void didPressedViewBegir(ChatBaseCell cell);
    ...
    private boolean ViewBegirPressed;
    private int ViewBegirStartX;
    private int ViewBegirStartY;
    ...
    public void setMessageObject(MessageObject messageObject) {
        ...
        wasLayout = false;
        drawViewBegirButton = drawShareButton = checkNeedDrawShareButton(messageObject);
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (delegate == null || delegate.canPerformActions()) {
                if (isAvatarVisible && avatarImage.isInsideImage(x, y)) {
                    ...
                    result = true;
                    invalidate();
                } else if (drawViewBegirButton && x >= ViewBegirStartX && x <= ViewBegirStartX + AndroidUtilities.dp(40) && y >= ViewBegirStartY && y <= ViewBegirStartY + AndroidUtilities.dp(32)) {
                ViewBegirPressed = true;
                    result = true;
                    invalidate();
                }
                if (result) {
            ...
            if (avatarPressed) {
                ...
                invalidate();
            }else if (ViewBegirPressed) {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ViewBegirPressed = false;
                    playSoundEffect(SoundEffectConstants.CLICK);
                    if (delegate != null) {
                        delegate.didPressedViewBegir(this);
                    }
                } else if (event.getAction() == MotionEvent.ACTION_CANCEL) {
                    ViewBegirPressed = false;
                } else if (event.getAction() == MotionEvent.ACTION_MOVE) {
                    if (!(x >= ViewBegirStartX && x <= ViewBegirStartX + AndroidUtilities.dp(40) && y >= ViewBegirStartY && y <= ViewBegirStartY + AndroidUtilities.dp(32))) {
                        ViewBegirPressed = false;
                    }
                }
                invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawShareButton) {
            ...
            Theme.ViewBegirDrawable.setColorFilter(ViewBegirPressed ? Theme.colorPressedFilter : Theme.colorFilter);
            setDrawableBounds(Theme.ViewBegirDrawable, ViewBegirStartX = currentBackgroundDrawable.getBounds().right + AndroidUtilities.dp(8), ViewBegirStartY = layoutHeight - AndroidUtilities.dp(75));
//            Theme.ViewBegirDrawable.draw(canvas);
            setDrawableBounds(Theme.ViewBegirIconDrawable, ViewBegirStartX + AndroidUtilities.dp(9), ViewBegirStartY + AndroidUtilities.dp(9));
//            Theme.ViewBegirIconDrawable.draw(canvas);
        }
